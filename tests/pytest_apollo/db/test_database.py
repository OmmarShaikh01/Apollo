...
class Test_LibraryManager:
    ...
    def test_get_library_stats(self, get_library_manager: LibraryManager, mocker: pytest_mock.MockerFixture):
        db = get_library_manager
    ...
    def test_rescan_files(self, get_library_manager: LibraryManager, mocker: pytest_mock.MockerFixture):
        db = get_library_manager
        file_path = [MEDIA_FOLDER / 'mp3' / "example_48000H_2C_TAGGED.mp3" for _ in range(10)]
        items_id = list(range(1, len(file_path) + 1))
        mocker.patch.multiple("apollo.media.decoders.Stream", __abstractmethods__ = set(),
                              _get_stream_info = lambda x: dict(file_id = items_id.pop(0),
    ...
    def test_rescan_folder(self, get_library_manager: LibraryManager, mocker: pytest_mock.MockerFixture):
        db = get_library_manager
